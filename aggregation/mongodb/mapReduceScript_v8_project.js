 ...
 */
	...
	function mapServer(serverConf) {
		...
		//	runs the test with the arguments configured in the leave
		//	and if the test returns true constructs a report and adds it to the
		//	result tree (otherwise the result tree contains no report at this
		//	leave for this incoming document)
...
function reduceFact(key, values) {
	...
	values.forEach(function(value) {
		...
		//	then reduce autonomous systems
	...
	//	and then, fingers crossed, retun the reduced fact
	return fact;
	...
	db.import.mapReduce (
		...
		reduceFact,
		{
			out: {
				...
				//	'reduce' would add values to existing documents
				merge: "facts"
			},
			query: {
				"date": date,
