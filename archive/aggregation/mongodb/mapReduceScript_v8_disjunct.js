 ...
 *
 * 					embarrassingly the attempt to establish disjunctive and
 * 					projective testing programmatically didn't work out.
 * 					the culprit is probably the
 * 						function disjunct(testArgs, reportArgs)
 * 					maybe setting 'disjunctive' to false from within
 *					the loop doesn't propagate probably to the outside.
 * 					but other, even dumber mistakes can't be ruled out as well.
 * 					disjunct() in it's current - buggy - state returns the results
 * 					as project().
 *
 * 					since implementing the disjunctive version in projective logic
 * 					- "brute forcing" it, so to say - took only about half an hour
 * 					we finally gave up on the elegant, extendable programmatic
 * 					version.
 *
 *					see mapReduceScript_v8_project.js and it's successor
 *					mapReduceScript_v9... for further work on this script
 *
 * 					still: a shame...
 */
		...
		testArgs.forEach(function (config) {
			...
			for (var prop in config) {
				if (config.hasOwnProperty(prop)) {
					...
					//	that prop should not be absent from the serverConf args list
					//	and test positive
					if ( reportArgs.indexOf(config[prop]) < 0 ) {
						if (config[prop] === 1 ) {
							disjunctive = false;
						}
					}
					//	it should as well, if it occurs in the serverConf args list,
					//	not test negative
					else if ( reportArgs.indexOf(config[prop]) > -1 ) {
						if (config[prop] === 0) {
							disjunctive = false;
						}
					}
